...
(is (= 0.0 (diss (make-input :a [:a]) (make-input :a [:a]) (make-domains))))
(is (= 0.8 (diss (make-input :a [:a]) (make-input :a [:b]) (make-domains))))
(is (= 1.0 (diss (make-input :a [:a]) (make-input :b [:a]) (make-domains))))
(is (= 0.0 (diss (make-input [:a :a] [:a]) (make-input [:a :a] [:a]) (make-domains))))
(is (= 1.0 (diss (make-input [:a :a] [:a]) (make-input [:a :b] [:a]) (make-domains))))
(is (= 0.4 (diss (make-input :a [:a :a]) (make-input :a [:a :b]) (make-domains))))
(is (< 0.26 (diss (make-input :a '(a a)) (make-input :a '(a b)) (make-domains)) 0.27))
(is (< 0.53 (diss (make-input :a '(a a)) (make-input :a '(b a)) (make-domains)) 0.54))
(is (= 0.0 (diss (make-input :a [:a [:a]]) (make-input :a [:a [:a]]) (make-domains))))
(is (= 0.4 (diss (make-input :a [:a [:a]]) (make-input :a [:a [:b]]) (make-domains))))
(is (= 0.0 (diss (make-input :a [:a [:a :a]]) (make-input :a [:a [:a :a]]) (make-domains))))
(is (= 0.2 (diss (make-input :a [:a [:a :a]]) (make-input :a [:a [:a :b]]) (make-domains))))

(is (< 0.13 (diss (make-input :a [:a '(a a)]) (make-input :a [:a '(a b)]) (make-domains)) 0.14))

(let [ia1 (make-input :a [1])
      ia2 (make-input :a [2])
      ia3 (make-input :a [3])
      ib1 (make-input :b [0])
      ib2 (make-input :b [5])
